...
export function useSendOffer() {
  ...
  async function send(
    ...
    labPrepHours: number,
    gradingHours: number,
    hasUnavailabilityMatch: boolean,
    onSuccess: () => void
  ) {
    const error = validate(app, need, hasUnavailabilityMatch, gradingHours, labPrepHours, sectionHours);
    if (error) {
    ...
    type TaskPayload = { task: AllocationType; hours: number };
